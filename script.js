...
let ball = {
  ...
  y: Math.round(canvas.height - canvas.height / 9),
...
ballInit.vy = - Math.round((Math.random() * ballInit.velocity / 2 + ballInit.velocity / 2) * 100) / 100;
ballInit.vx = Math.round(Math.pow(ballInit.velocity * ballInit.velocity - ballInit.vy * ballInit.vy, 0.5) * 100) / 100 * Math.random() >= 0.5 ? -1 : 1;
// ballInit.angle = Math.atan(ballInit.vx / ballInit.vy) / Math.PI * 180;
// ballInit.velocity = Math.pow(ballInit.vx * ballInit.vx + ballInit.vy * ballInit.vy, 0.5);  // Определяется ли где-нибудь скорость мяча?
...
brickInit.width = (canvas.width - (brickInit.lineCount - 1) * brickInit.gapWidth - brickInit.startX * 2) / brickInit.lineCount;
...
for (let l = 0; l < brickInit.lineCount; l++) {
  ...
  let x = brickInit.startX + l * (brickInit.width + brickInit.gapWidth);
  // let y = brickInit.gapHeight;
  let y = brickInit.startY;
...
function draw() {
  ...
  frame.elapsed = Date.now() - frame.last;

  // console.log(frame.elapsed);

  if (frame.elapsed < frame.length) {
    sleep(Math.floor(frame.length - frame.elapsed));
  }
  // else { console.log('!') }
  ...
  let directionChange = { // Запрос на изменение направления движения
    vx: false,
    vy: false
  }
  ...
  if (directionChange.vx == true) {
    ball.vx = -ball.vx;
    directionChange.vx = false
  }

  if (directionChange.vy == true) {
    ball.vy = -ball.vy;
    directionChange.vy = false
  }
